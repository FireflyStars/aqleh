...
$('#service-desc-en-editor').html($('#edit-service-form input[type="hidden"]#desc').val());
$('#service-desc-ar-editor').html($('#edit-service-form input[type="hidden"]#desc_ar').val());
$('.dropify').dropify({
    messages: {
...
$(document).ready(function(){
    ...
    $('#edit-service-form #submit').on('click', function(e){
        ...
        if ($parsley.validate()) {
            ...
            // Get form
            var form = $('#edit-service-form')[0];

            // Create an FormData object 
            var data = new FormData(form);
            // add spinner
            if ($('#edit-service-form input[type="checkbox"]#status').prop('checked')) {
                data.set('status', 1);
            }else
                data.set('status', 0);

            $("#edit-service-form #submit span").addClass('spinner-grow');
            // disabled the submit button
            $("#edit-service-form #submit").prop("disabled", true);
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $("#edit-service-form #submit span").removeClass('spinner-grow');
                    $("#edit-service-form #submit").prop("disabled", false);
                },
                error: function (e) {
                    console.log("ERROR : ", e);
                }
