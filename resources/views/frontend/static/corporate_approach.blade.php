...
@section('content')
{{--  page banner  --}}
	<section class="page-banner" id="corporate-approach-banner" style="background:url({{ session()->get('banner_url') }}) center center/cover no-repeat local">
		<div class="container">
			<div class="title-box">
				<h1 class="text-capitalize">{{ __('message.Corporate_Approach') }}</h1>
				<ol class="breadcrumb m-0 p-0 justify-content-center">
					<li><a href="{{ route('home') }}">{{ __('message.home') }}</a></li>
					<li><a>{{ __('message.aboutus') }}</a></li>
					<li>{{ __('message.Corporate_Approach') }}</li>
				</ol>
			</div>
		</div>
	</section>
{{--  home start  --}}
	<section class="section pt-5 pb-5" id="corporate-approach">
		<div class="container">
			<div class="row">
				<div class="col-8">
					<div class="corporate-approach-desc {{ app()->getLocale() == 'ar' ? 'lang-ar' : '' }}">
						{{ $page->desc }}
					</div>
				</div>
...
@section('js')
	<script>
		$('.corporate-approach-desc').html($('.corporate-approach-desc').text());
	</script>
