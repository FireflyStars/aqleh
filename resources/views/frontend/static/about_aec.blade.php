...
@section('content')
{{--  page banner  --}}
	<section class="page-banner" id="about-aec-banner" style="background:url({{ session()->get('banner_url') }}) center center/cover no-repeat local">
		<div class="container">
			<div class="title-box">
				<h1 class="text-capitalize">{{ __('message.about_AEC') }}</h1>
				<ol class="breadcrumb m-0 p-0 justify-content-center">
					<li><a href="{{ route('home') }}">{{ __('message.home') }}</a></li>
					<li><a>{{ __('message.aboutus') }}</a></li>
					<li>{{ __('message.about_AEC') }}</li>
				</ol>
			</div>
		</div>
	</section>
{{--  page banner  --}}
{{--  start  --}}
	<section class="section pt-5 pb-5" id="about-AEC">
		<div class="container">
			<div class="row">
				<div class="col-lg-8 col-md-8 about-aec-desc {{ app()->getLocale() == 'ar' ? 'lang-ar' : '' }}">
					{{ $page->desc }}
				</div>
				<div class="col-lg-4 col-md-4 d-flex justify-content-center">
					<ul class="navbar-nav d-block custom-nav">
						<li class="nav-item active big-divider">
...
@section('js')
	<script>
		$('.about-aec-desc').html($('.about-aec-desc').text());
	</script>
