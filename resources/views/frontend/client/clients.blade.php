...
{{-- Banner Start --}}
	...
	<section class="page-banner" id="clients-banner" style="background:url({{ session()->get('banner_url') }}) center center/cover no-repeat local">
		<div class="container">
			<div class="title-box">
				<h1 class="text-capitalize">{{ __('message.clients') }}</h1>
				<ol class="breadcrumb m-0 p-0 justify-content-center">
					<li><a href="{{ route('home') }}">{{ __('message.home') }}</a></li>
					<li>{{ __('message.clients') }}</li>
				</ol>
			</div>
		</div>
	</section>
...
{{-- Project Section Start --}}
	<section class="clients pt-5" style="position:relative">
		<div class="container">
			<div class="row">
				<div class="col-12 pb-4" style="white-space: pre-line">
					{{ __('message.client_summary') }}
				</div>
			</div>
			<div class="row">
				@foreach ($clients as $client)
					<div class="col-lg-3 col-sm-4">
						<div class="client-box text-center mb-3">
							<img src="{{ $client->img_url }}" alt="" class="img-responsive">
						</div>
					</div>
				@endforeach
			</div>
...
@endsection
@section('js')
<script src="{{ asset('assets/js/frontend/pages/client/clients.init.js') }}"></script>
@endsection